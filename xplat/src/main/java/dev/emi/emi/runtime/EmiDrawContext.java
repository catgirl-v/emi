...
public class EmiDrawContext {
	...
	public void drawOrderedTooltip(List<OrderedText> text, int x, int y) {
		List<TooltipComponent> components = text.stream().map(TooltipComponent::of).collect(Collectors.toList());
		((ScreenAccessor) client.currentScreen).invokeRenderTooltipFromComponents(matrices, components, x, y);
	}
}
